...
Есть два способа начать работу с PyEngine.

### Способ 1: Через pip (рекомендуется для пользователей)
Когда библиотека будет опубликована на PyPI, ты сможешь установить её одной командой:

```bash
pip install pyengine-x
```

### Способ 2: Ручная установка (для разработчиков и тестирования)
Скопируй папку `pyengine` из этого репозитория в папку своего проекта. Затем импортируй нужные модули:
